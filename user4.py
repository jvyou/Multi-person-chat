...
class Second(Ui_MainWindow2,QtWidgets.QMainWindow):
    ...
    def registers(self):
        ...
        tou='touxiang.jpg'
        if useres == '':
            reply = QMessageBox.information(self, '标题', '账号不许为空，注册失败！',
                                            QMessageBox.Ok)  # 信息框
            return False
...
class Third(Ui_MainWindow3,QtWidgets.QMainWindow):
    ...
    def alters(self):
        ...
        password_s = self.lineEdit_3.text()
        if useres == '':
            reply = QMessageBox.information(self, '标题', '账号不许为空，修改失败！',
                                            QMessageBox.Ok)  # 信息框
            return False
        if password == '' or password_s == '':
        ...
        for i in range(len(all_row)):
            if (all_row[i][0] == useres):
                ...
                break
        if flag==1:
            reply = QMessageBox.information(self, '标题', '未找到此账号，修改失败！',
                                            QMessageBox.Ok)  # 信息框
            self.lineEdit.clear()
            self.lineEdit_2.clear()
            self.lineEdit_3.clear()
...
class Fourth(Ui_MainWindow4,QtWidgets.QMainWindow):
    ...
    def backs(self):
        useres = self.lineEdit.text()
        if useres == '':
            reply = QMessageBox.information(self, '标题', '账号不许为空，找回失败！',
                                            QMessageBox.Ok)  # 信息框
            return False
        sql = "select * from users"
        cur.execute(sql)
        ...
        for i in range(len(all_row)):
            if (all_row[i][0] == useres):
                ...
                break
        if flag==1:
            reply = QMessageBox.information(self, '标题', '未找到此账号，找回失败！',
                                            QMessageBox.Ok)  # 信息框
            self.lineEdit.clear()
            return False
